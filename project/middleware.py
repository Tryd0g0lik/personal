...
from person.models import User, TypeUserModel
...
class RedisAuthMiddleware:
    ...
    async def async_get_user(
        ...
        try:
            ...
            # dcode = DcodeManager()
            #
            # b_session_key = dcode.bynary_to_str(session_key)
            # user_obj = dcode.binary_to_object(b_session_key)
            #
    ...
    def __call__(self, request: HttpRequest) -> HttpRequest:
        ...
        try:
            if request.COOKIES:
                ...
                if token_access or token_refresh:
