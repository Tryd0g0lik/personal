...
#             and not request.user.groups.filter(name__in=["ADMIN", "Supervisor"]).exists()
#         )
# class IsReaderALL(BasePermission):
#     """allows access only for read"""
#
#     def has_permissionps(self, request: Request, view=None) -> bool:
#         return (
...
#             or not request.user.groups.filter(name__in=["ADMIN", "Supervisor"]).exists())
#         )
#
#
...
#         return (
#             IsActive().has_permission(request)
#             and request.user.groups.filter(name__in=["ADMIN", "Supervisor"]).exists()
#         )
